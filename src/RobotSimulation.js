export default class RobotSimulation {
  /**
  ...
  inverse(x, y, z, a, b, c) {
    ...
    R[0] += Math.PI / 2 - Math.acos(this.J_initial_absolute[4][2] / RobotSimulation.length2(J[4][2], J[4][0]))
    R[0] += Math.atan2(-J[4][2], J[4][0])

    if (this.J_initial_absolute[4][2] > RobotSimulation.length2(J[4][2], J[4][0]) && this.debug) {
    ...
    const J2J4_length_x_y = RobotSimulation.length2(this.geometry[2][0] + this.geometry[3][0], this.geometry[2][1] + this.geometry[3][1])
    ...
    const J4J5_J4J3_normal_vector = RobotSimulation.cross(J4J5_vector, J4J3_vector)
    const XZ_parallel_aligned_vector = [
      10 * Math.cos(R[0] + (Math.PI / 2)),
      0, -10 * Math.sin(R[0] + (Math.PI / 2)),
    ]

    const reference = RobotSimulation.cross(XZ_parallel_aligned_vector, J4J3_vector)

    R[3] = RobotSimulation.angleBetween(J4J5_J4J3_normal_vector, XZ_parallel_aligned_vector, reference)

    // ---- R4 ----
    // # J4 J3 J5

    const referenceVector = RobotSimulation.cross(J4J3_vector, J4J5_J4J3_normal_vector)

    R[4] += RobotSimulation.angleBetween(J4J5_vector, J4J3_vector, referenceVector)
    ...
    R[5] += Math.PI / 2
    R[5] -= RobotSimulation.angleBetween(J4J5_J4J3_normal_vector, targetVectorY, RobotSimulation.cross(targetVectorY, targetVectorX))
  ...
  static angleBetween(vectorA, vectorB, referenceVector) {
    ...
    const norm = RobotSimulation.length3(RobotSimulation.cross(vectorA, vectorB))
